...
**8/22 UI foundations pass (against apple-design / emil-design-eng; zero functional change)**: ① A real bug — Home scrolled horizontally at every laptop width between 1101 and ~1600, carrying all three Approve buttons off-screen at 1280 and 1440 (measured scrollWidth 1483 / 1489). Both causes were rules that existed only inside the ≤640 media query: a grid item's default `min-width: auto`, and `.num` conflating tabular digits with nowrap. Both promoted to all widths; zero horizontal scroll verified at 1100 / 1280 / 1440 / 1680 and at 390. ② Tokens — `--canvas` and `--surface` were both `#f2f2f3`, so cards never lifted off the page ground and every `--shadow-*` was invisible. Separated into three steps (sunk ground / lightest card surface / sunken well inside a card). The zero radii are **kept as the deliberate decision they are** and documented as such. ③ Hierarchy — the Executive amber banner plus four standing paragraphs became one collapsed disclosure (content intact; the guarantee they describe is still enforced by the blocks that print a reason in place of a number). Coverage stopped explaining its own debounce interval, keeping only the live state. Home footers carrying sentences and links left the 9.5px mono uppercase face. ④ Colour discipline — Decline is no longer red (red means the steel is genuinely absent); the affirmative settled on `--accent`; Surplus stopped colouring its majority state. ⑤ MOR month axis — eighteen colliding labels became every third month labelled with ticks between, and `en-US` replaced `en-GB`'s wider "Sept". ⑥ Home's three columns followed three different conventions (capped with a footer / linked from the title / unbounded), so their heights and the rules between them ended at three places; the change feed took the same cap and footer as its neighbour and the columns now stretch to a common height. ⑦ Buttons — six near-identical declarations of the same affirmative action collapsed into one, resolving a `--brand` / `--accent` split. **Deliberately unchanged**: the loud dashed-amber `exec-unavailable`. Its comment states the reason — a tidy gap reads as a healthy zero — and that outranks the observation that it looks alarming.

**8/22 adoption from the sibling implementation (backend)**: a parallel rebuild of this platform exists in another repository (2026-08-19/20, 322 tests). It was reviewed and **only the three things it had that this codebase lacked** were taken. ① `PATCH /business-units/{id}` (rename; the BU id is what every pool resolves through and the name is presentation, so no recompute — pinned by asserting the stored verdicts are unchanged). ② `DELETE /business-units/{id}` (**the refusal is the feature**: 409 with itemised counts while any customer, on-hand row, on-order row, upload, user or scenario override still points at it; no cascade and no force flag). ③ `POST /coverage/recompute` (all customers or one; **recomputed and committed per customer, so a customer that cannot be evaluated is rolled back alone and returned by name with its reason** — the read path's `_recompute_all` cannot be reused here because it assumes its caller always rolls the whole transaction back; takes no scope filters, because the official verdict is the default-scope one). **Not taken**: the sibling models Business Units as a parent/child tree with cycle-checked reparenting, but this platform is flat on purpose (`coverage.py` states the pool is a flat join needing no recursive walk), and a hierarchy no engine reads is a field an operator can set and be misled by (pinned by `test_rename_is_the_only_editable_field_on_a_business_unit`). ④ **SPA navigation generalised** — the sibling had inverted this same middleware from "enumerate every client route" to "exempt the few that must answer for themselves", and that inversion is right: our `_SPA_ROUTES` carried a "KEEP IN SYNC with the <Routes> table" warning and that sync had already failed once (bare /wells missing, a trimmed URL landing on a JSON dead end, QA 8/14). Adopted, so **adding a screen no longer touches main.py**. Its version, however, exempts only the API docs and would therefore swallow its own file downloads (an `<a href>` arrives HTML-first, indistinguishable from a screen reload); here the exemptions are the docs, the health probe and anything ending in `/template` or `/export`, and `Vary: Accept` is kept — it is the fix for a real bug where the browser answered the SPA's fetch from the cached navigation and the coverage screen died parsing HTML as JSON. `test_spa_navigation.py` (16 tests) pins the shell for a route no router knows about, JSON for the fetch of the same path, and the download staying a spreadsheet. Its security posture is identical — `require_admin` unapplied, the AUTH_SECRET dev fallback, no object-level authorization — so there was nothing to gain there. 666 tests pass.
...
## 19. 2026-09-06: the adversarial review — package 2, "get the numbers and the record right"

**Implemented (4 commits, 720 → 739 tests, two migrations `b8d3e7f19c42` and `c7e2a9d41f58`, applied to dev.db with every customer recomputed)**:
1. **F05** Setting, creating or deleting an assignment re-judges **every customer in the BU** through `_recompute_assignment_scope` (a customer with no BU falls back to itself). This closes the reproduced case where another customer's stored verdict stayed stale.
2. **F06** "A decided approval is immutable" moved from an API pre-check into **`decide_approval` itself** (`ApprovalAlreadyDecided`; the API only translates it to 409). The hole where `scenario_apply` flipped Approved back to Pending and cleared `decided_at` is refused up front by `apply_blockers`, so the preview shows the blocker too. Restating the same verdict is a no-op; Approved on a Rejected pair opens a **new superseding row** (the Rejected row stays as history); Rejected on an Approved pair is refused, because under the resolution rule ("Approved beats any later re-request") a new Rejected row would change nothing, and saying so is the honest answer. Apply now reads the same row the verdict reads (`effective_approval`: newest, Approved wins).
3. **F09** The actor is recorded server-side: `created_by_user_id` on scenarios, `override_approved_by_user_id` on import rows, `requested_by_user_id` / `decided_by_user_id` on approvals, and a new append-only `company_inventory_edits` table (row, action, before/after, user, time) for inline inventory writes. The free-text `created_by` / `approved_by` stay as "on behalf of" and the screens label them that way. **No foreign key on the actor columns**: a record of who acted must survive that user being removed. Body spoofing is pinned by tests.
4. **F04** Coverage stores, per line, demand / drawn from customer-owned / drawn from company (pool + own assignment) / drawn from substitute / **residual** on `CoverageResult`, and MRP sums the residual. 5,000 demanded against 4,000 on hand now recommends 1,000 (it was 5,000: the 4,000 counted both as consumed stock and as steel to order). The scenario preview hands MRP its own pass's breakdown in the same shape (`LineNet`). Verdict rows that predate the columns are counted whole and the reason says to recompute. The MRP screen and the Excel summary gained Net shortfall / Demand / Drawn columns. **MOR is untouched**: its monthly ledger already nets opening stock against demand month by month, so feeding it the residual would net twice.
