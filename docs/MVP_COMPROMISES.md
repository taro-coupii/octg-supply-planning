...
| C-17 | 🟡 | (meaning of a figure) | **Double draw on a substitute-rescued line.** The own-product partial draw is kept and the whole demand is charged against the substitute (`coverage.py` fall-through, `remaining_qty[sub] -= line.quantity`), so the breakdown F04 stores (`drawn_company` + `drawn_substitute`) can exceed the demand on such a line. No effect on MRP (residual = 0). Whether to release the partial draw or charge the substitute only the remainder awaits an owner ruling. Implementation: `app/engines/coverage.py` |
