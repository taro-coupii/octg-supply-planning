...
| C-13 | 🟡 | ③ the BU is an absolute boundary | **Mostly closed 2026-09-06.** `app/auth/scope.py` walks every id in the path, query and JSON body back to its BU and answers 403 with zero writes; lists filter by `planner_bu`; a request naming several resources is refused whole if one is foreign (owner ruling). The remaining `/mrp/by-item` hole was closed the same day as C-15. Tests: `test_resource_scope_authz.py` / `test_list_scope_authz.py` |
| C-15 | ✅ | ③ the BU is an absolute boundary | **Closed 2026-09-06.** `by_item(db, product_id, business_unit_id=)` confines both the inventory position and the demand charged to it to one BU, and `/mrp/by-item` takes `planner_bu`. `/mrp/lead-time` is master data and reads no stock, so it needs no scope. Test: `test_list_scope_authz.py::test_by_item_is_the_planner_bu_position` |
...
| C-17 | ✅ | (meaning of a figure) | **Closed 2026-09-06 (owner ruling: "release the partial draw, substitute covers the whole line").** When a substitute rescues a line, its partial draws from the own-product pool and the customer's own stock go back to `remaining_qty` (available to later substitute draws in the same pass); an Oracle assignment is not released but no longer counts as drawn. The breakdown now equals the demand. Test: `test_substitute_releases_partial_draw.py` |
| C-18 | 🟡 | ③ the BU is the unit of allocation | **One unknown on-hand quantity leaves every customer of that Business Unit unevaluated.** (Owner ruling 2026-09-06: keep it.) Once the pool is divided in one pass, a part that cannot be measured means the division cannot be measured -- which is arithmetically honest. D01 widened the blast radius from "customers demanding that product" to "every customer sharing that pool". Every skipped customer is named in the response WITH ITS OWN REASON (fixed 2026-09-06, so no screen has to guess the cause). **Residual risk**: a day of missing Oracle feed blanks a wide area of the screens. Revisit per-product isolation once real data shows how often it happens. Implementation: `recompute_all_business_units` in `app/engines/coverage.py`, `_recompute_all` in `app/engines/coverage_view.py` |
...
- **C-15 closed too**: `by_item` takes a BU argument; `/mrp/by-item` serves the planner's BU position and demand only. `/mrp/lead-time` is master data and stays as is
